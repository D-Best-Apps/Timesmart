...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Set TagID to NULL if empty
    $tagID = empty($tagID) ? NULL : $tagID;
    ...
    // Check for duplicate TagID if provided (only if not NULL)
    if ($tagID !== NULL) {
   ...
   $hashed = password_hash($password, PASSWORD_BCRYPT, ['cost' => 12]);
    ...
    $stmt = $conn->prepare("
        INSERT INTO users (FirstName, LastName, Email, TagID, Office, ClockStatus, Pass, TwoFAEnabled, AdminOverride2FA)
        VALUES (?, ?, ?, ?, ?, 'Out', ?, 0, 1)
    ");
    $stmt->bind_param("sssssss", $firstName, $lastName, $email, $tagID, $office, $hashed);
